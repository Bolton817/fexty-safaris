...
export default function StickyBookingSidebar({ kshPrice, usdPrice, duration, packageTitle }: StickyBookingSidebarProps) {
  ...
  const handleBooking = (e: React.FormEvent) => {
    ...
    const query = new URLSearchParams({
      subject: `Booking Inquiry: ${packageTitle}`,
      date: date,
      guests: guests
    }).toString();
      ...
      <form className="space-y-4" onSubmit={handleBooking}>
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-3 bg-sand-50 border border-sand-200 rounded-xl focus:outline-none focus:border-sunset-500 transition-all text-sm" 
          />
        </div>
        <div className="relative">
